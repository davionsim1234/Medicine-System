...
def measure_search_time(search_function, search_keys: list[str]) -> int:
    """Measure the total execution time for a group of search operations."""
    start_time = perf_counter_ns()

    # Each key is searched so the timing covers multiple existing or non-existing records.
...
def run_performance_comparison(hash_table: HashTable, medicines: list[Medicine]) -> None:
    ...
    existing_keys = ["P0001", "P0250", "P0500", "P0750", "P1000"] * 1000
    non_existing_keys = ["P9999", "P0000", "P1200", "P2500", "P8888"] * 1000
    all_search_keys = existing_keys + non_existing_keys

    hash_existing_time = measure_search_time(performance_table.search, existing_keys)
    array_existing_time = measure_search_time(
        lambda key: linear_array_search(performance_medicines, key),
        existing_keys,
    )

    hash_missing_time = measure_search_time(performance_table.search, non_existing_keys)
    array_missing_time = measure_search_time(
        lambda key: linear_array_search(performance_medicines, key),
        non_existing_keys,
    )

    hash_total_time = measure_search_time(performance_table.search, all_search_keys)
    array_total_time = measure_search_time(
        lambda key: linear_array_search(performance_medicines, key),
        all_search_keys,
    )
    ...
    print(f"Existing-key searches          : {len(existing_keys)}")
    print(f"Non-existing-key searches      : {len(non_existing_keys)}")
    print(f"Total searches performed       : {len(all_search_keys)}")
    print("\nSearch type       | Hash table time    | Array time      | Hash avg     | Array avg")
    print("-" * 86)
    print(
        f"Existing keys     | {hash_existing_time:>15} ns | {array_existing_time:>12} ns | "
        f"{hash_existing_time / len(existing_keys):>9.2f} ns | "
        f"{array_existing_time / len(existing_keys):>9.2f} ns"
    )
    print(
        f"Non-existing keys | {hash_missing_time:>15} ns | {array_missing_time:>12} ns | "
        f"{hash_missing_time / len(non_existing_keys):>9.2f} ns | "
        f"{array_missing_time / len(non_existing_keys):>9.2f} ns"
    )
    print(
        f"Combined          | {hash_total_time:>15} ns | {array_total_time:>12} ns | "
        f"{hash_total_time / len(all_search_keys):>9.2f} ns | "
        f"{array_total_time / len(all_search_keys):>9.2f} ns"
    )
    print("\nAnalysis:")
    print(
