...
def prompt_positive_integer(message: str) -> int:
    """Keep requesting input until the user enters a positive whole number."""
    while True:
...
def prompt_non_empty_text(message: str) -> str:
    """Reject blank text fields so every transaction has useful details."""
    while True:
        value = input(message).strip()
        if value:
            return value
...
def prompt_positive_amount(message: str) -> float:
    """Accept only a positive, finite numeric transaction amount."""
    while True:
        value = input(message).strip()
        try:
            amount = float(value)
            ...
            print("Invalid amount. Enter a positive number, for example 49.90.")


def prompt_valid_date(message: str) -> str:
    ...
    while True:
        ...
        try:
            ...
            print("Invalid date. Use YYYY-MM-DD and enter a real calendar date.")


def prompt_transaction(transactions: list[Transaction]) -> Transaction:
    """Collect and validate transaction details for the optional insert feature."""
    # A transaction ID must be numeric, positive, and unique.
    while True:
        transaction_id = prompt_positive_integer("Enter transaction ID: ")
        if all(item.transaction_id != transaction_id for item in transactions):
            break
        print("That transaction ID already exists. Please enter a unique ID.")

    # Text, amount, and date fields are checked before creating the entity.
    customer_name = prompt_non_empty_text("Enter customer name: ")
    product_name = prompt_non_empty_text("Enter product name: ")
    amount = prompt_positive_amount("Enter amount: RM ")
    transaction_date = prompt_valid_date("Enter date (YYYY-MM-DD): ")
    ...
    while True:
        ...
        if choice == "1":
            ...
            if not sorted_transactions:
                sorted_transactions = merge_sort(transactions)
            target_id = prompt_positive_integer("Enter transaction ID to search: ")
            ...
            transactions.append(prompt_transaction(transactions))
            sorted_transactions = []
...
def run_demo() -> None:
    ...
    print("\nLinear Search for non-existing ID 9999:")
    print(linear_search(transactions, 9999))
    show_complexity_table()
    compare_performance(transactions)
